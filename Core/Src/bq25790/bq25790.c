...
static HAL_StatusTypeDef bqWriteReg(byte regAdrss, byte data, byte len){
	return i2cBqWrite(BQ_ADRS, regAdrss, &data, len);
}

static HAL_StatusTypeDef bqReadReg(byte regAdrss, byte *p, byte len){
	return i2cBqRead(BQ_ADRS, regAdrss, p, len);
}
 ...
 */
static void bqSetReg(byte regAdrss, byte startBit, byte mask, byte val)
{
	byte tmp;

	bqReadReg(regAdrss, &tmp, sizeof(tmp));

	tmp = tmp & (byte)~(mask << startBit);
	tmp = tmp | (byte)(val << startBit);

	bqWriteReg(regAdrss, tmp, sizeof(tmp));
}

void bqReadPartInfo(struct bq25790PartInfo *p){
	byte val;

    bqReadReg(REG48_Part_Information, &val, sizeof(val));
...
void bqReset(void){
    bqSetReg(REG09_Termination_Control, 6, 0x01, 1);
}
...
void bqEnOrMaskInterrupt(qbyte targIntMsk, qbyte bqIntMsk, byte intrMskReg, byte startBit, byte enOrDis){
	if(targIntMsk & bqIntMsk){
		bqSetReg(intrMskReg, startBit, 1, !enOrDis);
	}
}
...
void bqReadStatus1_3(struct bq25790Status1_3 *p){
    ...
    bqReadReg(REG1B_Charger_Status_0, &val0, sizeof(val0));
    bqReadReg(REG1C_Charger_Status_1, &val1, sizeof(val1));
    bqReadReg(REG1D_Charger_Status_2, &val2, sizeof(val2));
    bqReadReg(REG1E_Charger_Status_3, &val3, sizeof(val3));
...
void bqReadTempStatus(struct bq25790Status4 *q){
	...
	bqReadReg(REG1F_Charger_Status_4, &val4, sizeof(val4));
...
void bqReadStatusFault(struct bq25790StatusFault *r){
	...
	bqReadReg(REG20_FAULT_Status_0, &val5, sizeof(val5));
	bqReadReg(REG21_FAULT_Status_1, &val6, sizeof(val6));
...
void bqSetWatchdogTimer(enum bq25790Watchdog opt){
	bqSetReg(REG10_Charger_Control_1, 0, 0x07, opt);
}
...
void bqSetMinimalSystemVoltageLimit(qbyte volts){
	...
	bqSetReg(REG00_Minimal_System_Voltage, 0, 0x1F, (byte)regVal);
}
...
void bqSetChargeVoltageLimit(qbyte volts){
	...
	bqSetReg(REG01_Charge_Voltage_Limit, 0, 0xFF, lVal);
	bqSetReg(REG01_Charge_Voltage_Limit, 8, 0x07, hVal);
}
...
void bqSetChargeCurrentLimit(qbyte amperes){
	...
	bqSetReg(REG03_Charge_Current_Limit, 0, 0xFF, lVal);
	bqSetReg(REG03_Charge_Current_Limit, 8, 0x01, hVal);
}
...
void bqSetInputVoltageLimit(qbyte volts){
	...
	bqSetReg(REG05_Input_Voltage_Limit, 0, 0xFF, (byte)regVal);
}
...
void bqSetInputCurrentLimit(qbyte amperes){
	...
	bqSetReg(REG06_Input_Current_Limit, 0, 0xFF, lVal);
	bqSetReg(REG06_Input_Current_Limit, 8, 0x01, hVal);
}

void bqSetFastChargeVoltageThreshold(enum bq25790FastChargeThreshold opt){
     bqSetReg(REG08_Precharge_Control, 6, 0x03, opt);
}
...
void bqSetPrechargeCurrentLimit(qbyte amperes){
	...
	bqSetReg(REG08_Precharge_Control, 0, 0x3F, (byte)regVal);
}
...
void bqSetTerminationCurrent(qbyte mAmps){
   ...
   bqSetReg( REG09_Termination_Control, 0, 0x1F, (byte)regVal);
}
...
void bqSetRechargeThreshold(byte volts){
	...
	bqSetReg(REG0A_Recharge_Control, 0, 0x0F, (byte)regVal);
}
...
int16_t bqReadIBus(void){
	...
	HAL_StatusTypeDef sts = bqReadReg(REG31_IBUS_ADC, buf, sizeof(mAmps));
...
int16_t bqReadVBus(void){
	...
	HAL_StatusTypeDef sts = bqReadReg(REG35_VBUS_ADC, buf, sizeof(mVolts));
...
int16_t bqReadIBat(void){
	...
	HAL_StatusTypeDef sts = bqReadReg(REG33_IBAT_ADC, buf, sizeof(mAmps));
...
int16_t bqReadVBat(void){
	...
	HAL_StatusTypeDef sts = bqReadReg(REG3B_VBAT_ADC, buf, sizeof(mVolts));
...
int16_t bqReadVSys(void){
	...
	HAL_StatusTypeDef sts = bqReadReg(REG3D_VSYS_ADC, buf, sizeof(mVolts));
